...
namespace _Project.CodeBase.Gameplay.Services.Resource
{
  ...
  {
    bool TrySpend(ResourceKind kind, ResourceSink sink, int amount);
    void AddResource(ResourceKind kind, int amount);
    ReadOnlyReactiveProperty<int> ObserveResource(ResourceKind kind);

    void AddResourceDrop(ResourceDropType resourceDropType, Vector3 spawnPoint, Vector3 position);

    void CollectDrop(int id);
    ReadOnlyReactiveProperty<ResourceDropCollectedArgs> ResourceDropCollected { get; }
    bool TrySpend(ResourceSink sink, params ResourceAmountData[] resources);
    ...
    ReadOnlyReactiveProperty<int> ObserveCapacity(ResourceKind kind);
  }
}